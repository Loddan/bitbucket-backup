...
def clone_repo(repo, backup_dir, http, password, mirror):
    ...
    if scm == 'hg':
        if http:
            command = 'hg clone https://%s:%s@bitbucket.org/%s/%s %s' \
                        % (username, password, username, slug, backup_dir)
        else:
            command = 'hg clone ssh://hg@bitbucket.org/%s/%s %s' \
                        % (username, slug, backup_dir)
    if scm == 'git':
        if http:
            command = "git clone https://%s:%s@bitbucket.org/%s/%s.git %s" \
                        % (username, password, username, slug, backup_dir)
        else:
...
def main():
    ...
    if not username or not location:
        parser.error('Please supply a username and backup location'
                     ' (-u <username> -l <backup location>)')
    if not os.path.isdir(location):
        print('Backup location does not exist.  Please provide an '
              'existing directory.')
    bb = bitbucket.BitBucket(username, password)
    user = bb.user(username)
    repos = user.repositories()
    if not repos:
        print('No repositories found.  Are you sure you provided '
